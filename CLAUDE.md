...
- **orchestrator.js** — the brain. A `Phase` enum state machine (INTRO → ROCKET_EXIT (first tap: rocket morphs into the comet in place) → TRAVEL/VIEW through the trait text, rock constellation, attractor tour, and contact landing, then loops through the attractors). Phases advance on `tapPending` after a minimum time. Owns flight Bezier curves (`bezP0..3`), the world-space cursor `worldPos` (what the camera follows), and per-phase camera handoff. The landing traces `LANDING_LINES` (contact email) and parks an invisible `mailPlane` over the text that main.js raycasts for a `mailto:` click.
...
- **intro3d.js** — the 3D landing page (extruded name text, LinkedIn icon, rocket with idle loops). Objects live on a world-space anchor so view rotation moves them with the stars. On the first tap the text/icon stay put and only the rocket animates: `setRocketCharging` freezes its current idle transform and accelerates it along its own nose, stretching it into the comet streak (`getRocketTipWorld`/`getRocketDirWorld` hand the start point and heading to the orchestrator). Mode setters are driven by the orchestrator.
